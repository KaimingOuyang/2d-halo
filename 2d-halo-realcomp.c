...
static void delay(double *stencil, int dim)
{
    int c_e, n_e, s_e, w_e, e_e;
    ...
    for(int i = 1; i < dim - 1; ++i) {
        for(int j = 1; j < dim - 1; ++j) {
            ...
            w_e = i * dim + j - 1;
            e_e = i * dim + j + 1;
            stencil[c_e] = (stencil[c_e] + stencil[n_e] + stencil[s_e] + stencil[w_e] + stencil[e_e]) / 5.0;
        }
    }
    return;
}
...
int main(int argc, char **argv)
{
    ...
    while (--argc && ++argv) {
        if (!strcmp(*argv, "--iters")) {
            ...
            --argc;
            ++argv;
            computation = atoi(*argv) / comm_size;
        }
        else {
            usage();
        }
    ...
    double *stencil = malloc(sizeof(double) * dim * dim);
    memset(stencil, 0, sizeof(double) * dim * dim);
